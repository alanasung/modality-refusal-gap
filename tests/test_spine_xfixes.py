...
def test_mps_fallback_flag_in_device_info_dict():
    from vlmrefusal.models.device import DeviceInfo
...
def test_enable_mps_fallback_sets_env(monkeypatch):
    from vlmrefusal.models import device as device_mod
...
def test_apply_chat_template_raw_path():
    from vlmrefusal.models.generation import apply_chat_template
...
def test_role_keyed_config():
    from vlmrefusal.configs.schema import Config, ModelConfig
...
def test_data_default_n_items_pilot_sized():
    from vlmrefusal.configs.schema import DataConfig, EvalConfig
...
def test_validate_layers_rejects_gpt2_layer_12():
    from vlmrefusal.utils.validation import validate_layers
...
def test_registry_revision_pinned():
    from vlmrefusal.models.registry import MODEL_REGISTRY, get_model_spec
...
def test_mde_and_tost():
    from vlmrefusal.evaluation.metrics import Estimate, minimum_detectable_effect, tost_equivalence
...
def test_patch_activations_and_attn_exported():
    from vlmrefusal.models.hooks import intervene_attention_heads, patch_activations
...
def test_ablation_controls_and_dropout():
    from vlmrefusal.ablation import run_component_dropout, run_control_arms, run_seed_sweep
...
def test_reporting_roundtrip(tmp_path):
    ...
    from vlmrefusal.reporting import aggregate_results, write_figures, write_tables
    from vlmrefusal.utils.run_manifest import write_json_atomic
