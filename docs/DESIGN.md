...
|---|---|---|
| Config schema | `vlmrefusal.configs.schema` | Typed dataclasses + role-keyed models |
| Loader | `vlmrefusal.configs.loader` | Hydra compose API |
| Device | `vlmrefusal.models.device` | MPS/CUDA/CPU resolution + fallback flag |
| Registry | `vlmrefusal.models.registry` | Pinned revisions + hardware fit |
| Hooks | `vlmrefusal.models.hooks` | Capture, steer, ablate, patch, attn intervene |
| Generation | `vlmrefusal.models.generation` | Chat template path recording |
| Data | `vlmrefusal.data` | Synthetic items, splits, manifests |
| Evaluation | `vlmrefusal.evaluation` | Metrics, failure harness, protocols |
| Ablation | `vlmrefusal.ablation` | Control / dropout / seed-sweep arms |
| Reporting | `vlmrefusal.reporting` | Aggregate → MD/TeX/figures |
| Cache | `vlmrefusal.cache` | Versioned artifact store |
...
`src/vlmrefusal/vlmrefusal/` are preserved across regeneration.
